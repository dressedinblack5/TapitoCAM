...
while true; do
    ...
    ERROR_LOG=$(mktemp)
    mpv --log-file="$ERROR_LOG" --profile=fast --untimed --cache=no --demuxer-readahead-secs=0 --vd-lavc-threads=1 --rtsp-transport=udp --demuxer-lavf-o-add=fflags=+nobuffer --demuxer-lavf-o-add=probesize=5000000 --demuxer-lavf-o-add=analyzeduration=5000000 --video-sync=audio "rtsp://${TAPO_USER}:${TAPO_PASS}@${TAPO_IP}/stream1"
    MPV_EXIT_CODE=$?
